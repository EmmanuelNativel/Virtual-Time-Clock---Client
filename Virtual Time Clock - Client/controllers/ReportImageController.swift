    ...
    private func uploadImageToStorage(image:UIImage){
        ...
        uploadImageTask.observe(.failure) { snapshot in
            if let error = snapshot.error as NSError? {
                ...
                self.view.makeToast(NSLocalizedString("imageUploadError", comment: "Alert message"), duration: 3)
            }
        }
    }
    ...
    private func writeImagePathInBd(url: String){
        ...
        missionRef.updateData([
            ...
            if let err = err { print("⛔️ Erreur lors de l'écriture du chemin de l'image dans la BD : \(err)") }
                ...
                print("✅ L'url de l'image du rapport a bien été mis à jour dans la BD.")
                self.view.makeToast(NSLocalizedString("imageUploadSuccess", comment: "Alert message"), duration: 3)
            }
        }
    }
